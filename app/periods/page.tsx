...
export default function PeriodsPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Period Selection */}
      <div className="card">
        <h2 className="text-2xl font-bold mb-4 text-white">Создать новую оценку периода</h2>

        {/* Quick Period Buttons */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          <button
            onClick={() => selectQuickPeriod('week')}
            className={`p-4 rounded-lg border-2 transition-all ${ selectedPeriod?.type === 'week' ? 'bg-blue-600 text-white border-blue-500' : 'bg-gray-900/80 border-gray-700 hover:border-blue-400'}`}
          >
            <div className="text-2xl mb-1"></div>
            <div className="font-semibold text-white">Неделя</div>
          </button>

          <button
            onClick={() => selectQuickPeriod('month')}
            className={`p-4 rounded-lg border-2 transition-all ${ selectedPeriod?.type === 'month' ? 'bg-blue-600 text-white border-blue-500' : 'bg-gray-900/80 border-gray-700 hover:border-blue-400'}`}
          >
            <div className="text-2xl mb-1"></div>
            <div className="font-semibold text-white">Месяц</div>
          </button>

          <button
            onClick={() => selectQuickPeriod('quarter')}
            className={`p-4 rounded-lg border-2 transition-all ${ selectedPeriod?.type === 'quarter' ? 'bg-blue-600 text-white border-blue-500' : 'bg-gray-900/80 border-gray-700 hover:border-blue-400'}`}
          >
            <div className="text-2xl mb-1"></div>
            <div className="font-semibold text-white">Квартал</div>
          </button>

          <button
            onClick={() => selectQuickPeriod('year')}
            className={`p-4 rounded-lg border-2 transition-all ${ selectedPeriod?.type === 'year' ? 'bg-blue-600 text-white border-blue-500' : 'bg-gray-900/80 border-gray-700 hover:border-blue-400'}`}
          >
            <div className="text-2xl mb-1"></div>
            <div className="font-semibold text-white">Год</div>
          </button>
        </div>

        {/* Custom Period Selection */}
        <div className="rounded-lg p-4 border border-gray-700 bg-gray-800/50">
          <h3 className="font-semibold mb-3 text-white">Произвольный период</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div>
              <label className="block text-sm font-medium mb-1 text-gray-300">Начало</label>
              <input
                type="date"
                value={customStart}
            ...
            <div>
              <label className="block text-sm font-medium mb-1 text-gray-300">Конец</label>
              <input
                type="date"
                value={customEnd}
        ...
        {selectedPeriod && (
          <div className="mt-4 p-4 rounded-lg bg-gray-800/50 border border-green-500/30">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-400">Выбранный период:</p>
      ...
      <div className="card">
        <h2 className="text-2xl font-bold mb-4 text-white">История оценок</h2>
          ...
          <div className="space-y-3">
            {evaluations.map((evaluation) => (
              <Link
                ...
                className="block p-4 rounded-lg transition-all hover:shadow-md bg-gray-900/80 border-gray-700 hover:border-blue-400"
              >
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-lg text-white">{getPeriodLabel(evaluation.periodType)}</span>
                  ...
                  <div className="text-right">
                    <div className="text-sm text-gray-400 mb-1">Прогресс к мечте</div>
