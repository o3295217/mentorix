...
export default function TimeField({ value, onChange, disabled = false, ariaLabel, stepMinutes = 15, className = '' }: TimeFieldProps) {
  ...
  return (
    <>
      {/* Вне шкалы (весь компонент, как метки оси таймлайна): компактный HH:MM
          редактор и его выпадающий список часов/минут намеренно мельче типографической
          шкалы — это плотный числовой виджет, встраиваемый в узкие строки расписания,
          а не текст с читательской ролью. */}
      <input
        ref={inputRef}
        type="text"
        inputMode="numeric"
