...
Дисциплина обращений к браузеру (обязательно):
- Каждое действие в браузере — навигация, клик, снапшот, скриншот — заставляет MCP писать файл дампа на диск. Лишние действия засоряют проект и жгут контекст.
- Потолок на прогон: **не более 25 browser-действий и не более 6 скриншотов**. Упёрся в лимит — остановись и доложи, что успел проверить, а что нет. Не продолжай молча.
- Проверяй ТОЛЬКО те состояния и экраны, которых коснулось изменение. Полный перебор состояний из пункта 3 делай лишь тогда, когда lead потребовал явно.
- Не переснимай то, что уже снял. Один кадр на состояние. Если нужный элемент попал в предыдущий кадр — ссылайся на него, а не делай новый.
- Скриншоты — минимально достаточный набор для доказательства вердикта, а не отчёт о каждом клике. Обычно хватает 2-4: desktop, mobile и спорные места.
- Промежуточные снапшоты для навигации не сохраняй как evidence.
...
2. Проверь desktop и mobile viewports. Если lead не указал размеры, используй desktop ~1440×900 и mobile ~390×844.
3. Проверь страницы и states, затронутые изменением. Полный список возможных состояний — `default/loading/empty/error/success/focus/disabled/hover` — это справочник, а не обязательный чек-лист: бери из него только релевантное задаче. Нерелевантные состояния не открывай.
