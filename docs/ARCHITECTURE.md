...
Daily schedule proposal normalization is intentionally stronger than prompt validation: for proposal v3 the server sets `dayStartMinutes=planningStartMinutes` and `dayEndMinutes=activityEndMinutes`, rewrites task block text from the source of truth (`planTasks` for existing tasks during current-plan validation, `newTasks` for new tasks), keeps fixed blocks as immutable anchors, then packs flexible blocks in model-proposed order into free intervals. Flexible blocks that do not fit are removed from `blocks` and exposed through normalization metadata; `/api/daily/chat` adds a human SSE `text` note that those tasks stayed in «Не распределено». Two overlapping fixed blocks remain the deliberate hard-failure case for `DailyScheduleSchema`/corrective tool flow.
